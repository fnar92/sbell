...
.value('taOptions',  {
	...
	toolbar: [
		...
		['bold', 'italics', 'underline', 'strikeThrough', 'ul', 'ol', 'html', 'insertImage', 'insertLink'],
		['justifyLeft','justifyCenter','justifyRight','justifyFull','indent','outdent', 'redo', 'undo', 'clear']/*,
		...
		imgOnSelectAction: function(event, $element, editorScope){
			...
			remove.on('click', function(event){
				...
				$element.remove();
                                ...
                                (function(){ 
                                    console.log("eliminando $element-> ", $element[0].attributes[0].value);
                                })();
				finishEdit();
			});
...
.run(['taRegisterTool', '$window', 'taTranslations', 'taSelection', 'taToolFunctions', '$sanitize', 'taOptions', '$log', 'RestService', '$mdDialog', 'DialogService',
	function(taRegisterTool, $window, taTranslations, taSelection, taToolFunctions, $sanitize, taOptions, $log, RestService, $mdDialog, DialogService){
	...
	taRegisterTool('insertImage', {
		...
		action: function(){
			...
			$mdDialog.show({
			  ...
			  templateUrl: 'app/diagnostic/views/load.tpl.html',
			  parent: angular.element(document.body),
			  clickOutsideToClose:true
			})
			.then(function(answer) {
                            console.log("733 OK textAngularSetup", answer);
                            imageLink = answer.name;
                            check(imageLink);
			}, function() {
                            console.log("750 ERROR textAngularSetup");
			});
            var thisEditor = this;
            
//            var confirm = DialogService.dialogTpl(thisEditor, '', 'app/diagnostic/views/load.tpl.html', DialogController);
//            $mdDialog.show(confirm).then(function (answer) {
//                //imageLink = '../files/'+ answer.name;
//                imageLink = answer.name;
//		check(imageLink);
//            }, function () {
//                console.log("Error al subir imagen $mdDialog.show(confirm)");
//                //$mdDialog.hide();
//            });
			
			
			function check(imageLink){				
				if(imageLink && imageLink !== '' && imageLink !== 'http://'){
					...
					if (!blockJavascript(imageLink)) {
						...
						return thisEditor.$editor().wrapSelection('insertHTML', embed, true);
					}
				}
			}
        ...
        function DialogController($scope, $mdDialog) { // cuadro de dialogo
                            $scope.hide = function() {
                              $mdDialog.hide();
                            };
                            ...
                            $scope.load = function(answer) {                
                                var file = $scope.file;
                                var url = "../server.php";
                                RestService.upload(url, 'undefined', file)
                                    .then(function(res){
                                        console.info("textAngularSetup DialogController load-> ", res);
                                        //answer = res.data;
                                        answer = "aqui ok";
                                        $mdDialog.hide(answer);
                                    })
                                    .catch(function(err){
                                        console.info("ERROR textAngularSetup DialogController load-> ", err);
                                        //answer = "aqui_no.jpg";
                                        $mdDialog.hide(answer);
                                    });
                            };
