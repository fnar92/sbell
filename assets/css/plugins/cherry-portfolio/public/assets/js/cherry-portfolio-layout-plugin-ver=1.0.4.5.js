...
var
_this=$(this),ajaxGetNewRequest=null,ajaxGetMoreRequest=null,ajaxGetNewRequestSuccess=true,portfolioContainer=$('.portfolio-container',_this),portfolioList=$('.portfolio-list',portfolioContainer),ajaxLoaderContainer=null,column=portfolioContainer.data('column'),postPerPage=portfolioContainer.data('post-per-page'),itemMargin=parseInt(portfolioContainer.data('item-margin')),loadingMode=portfolioContainer.data('loading-mode'),fixedHeight=portfolioContainer.data('fixed-height'),listLayout=portfolioContainer.data('list-layout'),template=portfolioContainer.data('template'),postsFormat=portfolioContainer.data('posts-format'),isotopeOptions={itemSelector:'.portfolio-item',resizable:false,masonry:{columnWidth:Math.floor($('.portfolio-list',portfolioContainer).width()/ column ) }
},currentSlug='',currentPaginationPage=1,allPageLenght=0,allPageLenght_temp=0,ajaxMoreClicked=false,orderSetting={order:'DESC',orderby:'date'};_constructor();function _constructor(){if($('.cherry-portfolio-ajax-loader').length==0){portfolioContainer.append('<div class="cherry-portfolio-ajax-loader"><div class="cherry-spinner cherry-spinner-double-bounce"><div class="cherry-double-bounce1"></div><div class="cherry-double-bounce2"></div></div></div>');ajaxLoaderContainer=$('.cherry-portfolio-ajax-loader');}else{ajaxLoaderContainer=$('.cherry-portfolio-ajax-loader');}
...
ajaxGetNewContent(currentSlug,currentPaginationPage,orderSetting);allPageLenght=Math.ceil(parseInt($('.portfolio-list',portfolioContainer).data('all-posts-count'))/parseInt( postPerPage ) );
allPageLenght_temp=allPageLenght;addEventsFunction();}
...
var
elementsList=$('.portfolio-item',response),pagePagination=$('.portfolio-pagination',response),pageMoreButton=$('.portfolio-ajax-button',response),allPostsCount=$('.portfolio-list',response).data('all-posts-count');allPageLenght=Math.ceil(parseInt(allPostsCount)/ parseInt( postPerPage ) );
switch(listLayout){case'masonry-layout':case'grid-layout':$('.inner-wrap',elementsList).css({"margin":Math.floor(itemMargin*0.5)});$(elementsList).css({"width":Math.floor(portfolioList.width()/ widthLayoutChanger() ) });
portfolioList.html('').isotope(isotopeOptions).isotope('insert',elementsList);portfolioContainer.append(pagePagination);portfolioContainer.append(pageMoreButton);portfolioList.imagesLoaded(function(){portfolioList.isotope(isotopeOptions)
...
ajaxGetMoreRequest=$.ajax({type:'POST',url:portfolio_type_ajax.url,data:data,cache:false,beforeSend:function(){ajaxRequestSuccess=false;ajaxLoaderContainer.css({"display":"block"}).fadeTo(500,1);},success:function(response){ajaxRequestSuccess=true;ajaxLoaderContainer.fadeTo(500,0,function(){$(this).css({"display":"none"});});portfolioList=$('.portfolio-list',portfolioContainer);beforeItemLength=$('.portfolio-list .portfolio-item',portfolioContainer).length;var
elementsList=$('.portfolio-item',response),allPostsCount=$(response).data('all-posts-count');;allPageLenght=allPageLenght_temp;switch(listLayout){case'masonry-layout':case'grid-layout':var $elems=$(response);elementsList.css({"width":Math.floor($('.portfolio-list',portfolioContainer).width()/ widthLayoutChanger() ) });
$('.inner-wrap',elementsList).css({"margin":Math.floor(itemMargin*0.5)});portfolioList.append(elementsList).isotope('appended',elementsList);portfolioList.imagesLoaded(function(){portfolioList.isotope(isotopeOptions)
...
function ajaxMoreButtonClickEventFunction(){if(currentPaginationPage<allPageLenght){currentPaginationPage++;ajaxMoreClicked=true;if(currentPaginationPage==allPageLenght){$('.portfolio-ajax-button .load-more-button',_this).slideUp();}
...
;$('.portfolio-list .portfolio-item',portfolioContainer).css({"width":newWidth});$('.portfolio-list',portfolioContainer).isotope({masonry:{columnWidth:newWidth}});break;case'list-layout':break;}}
